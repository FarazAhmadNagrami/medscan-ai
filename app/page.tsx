      ...
      <div className="text-center mb-14">
        ...
        {!user && !loading && (
          <p className="mt-4 text-sm text-blue-600 dark:text-blue-400 font-medium">
            Click any module to get started — you&apos;ll be asked to sign in.
          </p>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        {modules.map((mod) => (
          <button
            ...
            className="group relative bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-5 hover:shadow-lg hover:-translate-y-1 transition-all duration-200 overflow-hidden text-left w-full"
          >
            ...
            {!user && !loading && (
              <div className="absolute inset-0 flex items-end justify-end p-3 opacity-0 group-hover:opacity-100 transition-opacity">
                <span className="text-xs px-2.5 py-1 bg-blue-600 text-white rounded-full font-medium shadow">
                  Sign in to use →
                </span>
              </div>
            )}
