...
function LoginForm() {
  ...
  // Handles email/password sign-in — popup sign-in redirects directly in handleGoogleSignIn
  useEffect(() => {
    if (user) {
  ...
  const handleGoogleSignIn = async () => {
    ...
    setBusy(true);
    try {
      // Popup resolves as a Promise — redirect immediately after it closes, no timing issues
    ...
    <div className="min-h-[85vh] flex items-center justify-center px-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white">MedScan AI</h1>
          <p className="text-gray-500 dark:text-gray-400 mt-2">
            {mode === "signin" ? "Sign in to access all features" : "Create your free account"}
          </p>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-8 shadow-sm">
          ...
          <button
            ...
            disabled={busy}
            className="w-full flex items-center justify-center gap-3 px-4 py-3 border border-gray-200 dark:border-gray-600 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors text-gray-700 dark:text-gray-300 font-medium mb-6 disabled:opacity-50"
          >
          ...
          <div className="relative mb-6">
            ...
            <div className="relative flex justify-center text-xs text-gray-400">
              <span className="bg-white dark:bg-gray-800 px-3">or with email</span>
            </div>
          </div>
        ...
        <p className="mt-6 text-center text-xs text-gray-400 dark:text-gray-600">
          MedScan AI is for informational purposes only — not a substitute for professional medical advice.
        </p>
      </div>
    </div>
  );
